...
def run_char_app():
    ...
    df2_result_final = df2.loc[(df2['시점']==2015) & (df2['신혼부부 특성별(1)']=='혼인1년차')& (df2['신혼부부 특성별(2)']=='혼인1년차'),'소득 평균(만원)'].values


    year_list = df2['시점'].unique()
    char_list1 = df2['신혼부부 특성별(1)'].unique()
    char_list2 = df2['신혼부부 특성별(2)'].unique()
    ...
    if selected_char1 == '혼인연차별':
        selected_char2 = st.selectbox('특성을 선택하세요.', df2.loc[(df2['신혼부부 특성별(2)'].str.contains('혼인')&(df2['시점']==selected_year)),'신혼부부 특성별(2)'])
    ...
    elif selected_char1 == '맞벌이여부':        
        selected_work=df2.loc[df2['신혼부부 특성별(1)']=='맞벌이여부','신혼부부 특성별(2)'].unique()
    ...
    st.success('해당 특성의 신혼부부의 평균소득은 ' + str(df2_result_final)+ '만원 입니다.')
